// lib/qrcode.js - 纯原生零依赖轻量二维码矩阵生成器 (兼容 Canvas & SVG 渲染)
(function (global) {
  'use strict';
  ...
  // 高阶 Canvas 与 SVG 渲染封装
  const QRCodeGenerator = {
    create: function (text, options = {}) {
      const ecLevelStr = (options.errorCorrectionLevel || 'M').toUpperCase();
      let ecLevel = QRErrorCorrectLevel.M;
      if (ecLevelStr === 'L') ecLevel = QRErrorCorrectLevel.L;
      else if (ecLevelStr === 'Q') ecLevel = QRErrorCorrectLevel.Q;
      else if (ecLevelStr === 'H') ecLevel = QRErrorCorrectLevel.H;

      const qr = new QRCodeModel(0, ecLevel);
      qr.addData(text || 'https://github.com');
      qr.make();
      return qr;
    },
    ...
    drawCanvas: function (canvas, text, options = {}) {
      ...
      const qr = this.create(text, options);
      ...
      for (let r = 0; r < moduleCount; r++) {
        for (let c = 0; c < moduleCount; c++) {
          if (qr.isDark(r, c)) {
            ...
            rects.push(`<rect x="${x}" y="${y}" width="${cellSize}" height="${cellSize}" fill="${darkColor}"/>`);
          }
        }
      }

      return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size} ${size}" width="${size}" height="${size}"><rect width="100%" height="100%" fill="${lightColor}"/>${rects.join('')}</svg>`;
    },
  ...
  if (typeof module !== 'undefined' && module.exports) {
    module.exports = { QRCodeGenerator, QRCodeModel };
  }
